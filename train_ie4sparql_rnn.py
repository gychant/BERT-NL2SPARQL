...
from rnn_model import RnnNTR
from glove import load_glove_model, load_w2v_model
...
qmark_placeholder = 'xxxxx'
uri_placeholder = 'yyyyy'
whitespace_placeholder = '[unused10]'
double_whitespace_placeholder = '[unused11]'

# warmup_linear_constant
parser = argparse.ArgumentParser()
parser.add_argument('--data_dir', default='sq-0726-2m-bubo-revised', help="Directory containing the dataset")
# lic-corrected-70-semantic-embedd-v4
# WebQSP_0823; WebQSP_0824_67
...
parser.add_argument('--weight_decay', default=0.005, help="")
parser.add_argument('--warmup', default=0.1, help="")
parser.add_argument('--debug', default=False, help="if use debug mode")
parser.add_argument('--model_dir', default='experiments_sq/lstm_trial1', help="model directory")
parser.add_argument('--epoch_num', default=1000, help="num of epoch", type=int) # sq:150->100, lcquad:600, sq:40
parser.add_argument('--batch_size', default=16, help="batch size") # sq:64, lcquad:16, sq:16
parser.add_argument('--max_len', default=300, help="max sequence length", type=int) # sq:300, lcquad:200, sq:300
parser.add_argument('--learning_rate', default=0.0005, help="learning rate") # 0.0005; 0.001 - 0.0001
parser.add_argument('--repeat_multi_sub', default=False, help="if to repeat multi subs instead of sampling")
# encoder_type: [bert, rnn]
parser.add_argument('--encoder_type', default="rnn", help="Encoder type")
parser.add_argument('--word_embedding', default="glove/glove.840B.300d.txt", help="word embedding dir file")
# glove/glove.6B.300d.txt, 42, 840
#rnn
parser.add_argument('--cell_type', default="gru", choices=['lstm', 'gru'], help="RNN cell_type")
parser.add_argument('--hidden_size', type=int, default=300, help="RNN hidden_size")
parser.add_argument('--num_layers', type=int, default=2, help="RNN num_layers")
parser.add_argument('--word_embed_dim', type=int, default=300, help="RNN word_embed_dim")
parser.add_argument('--type_embed_dim', type=int, default=128, help="RNN type_embed_dim")
parser.add_argument('--dropout', type=float, default=0.2, help="RNN dropout") # for both rnn and following linear layer
# Ema
...
# extraction
parser.add_argument('--dyna_thres', default=True, help="if dynamically extract during evaluation")
parser.add_argument('--sub_threshold', default=0.4, help="subject threshold during extraction")
parser.add_argument('--obj_threshold', default=0.4, help="object threshold during extraction")
...
def evaluate(model, data_loader, data, output_file_path):
    ...
    with torch.no_grad():
        ...
        for i in t:
            ...
            triplet_spans = model.extract(batch_tokens,
                                          sub_threshold=args.sub_threshold,
                                          obj_threshold=args.obj_threshold)
                                        # subj_thre = obj_thre = 0.4; shape:(batch_size, max_len)

            if args.dyna_thres:
                if len(triplet_spans) == 0:
                    triplet_spans = model.extract(batch_tokens,
                                                  sub_threshold=args.sub_threshold-0.1,
                                                  obj_threshold=args.obj_threshold)
                if len(triplet_spans) == 0:
                    triplet_spans = model.extract(batch_tokens,
                                                  sub_threshold=args.sub_threshold-0.1,
                                                  obj_threshold=args.obj_threshold-0.1)
                if len(triplet_spans) == 0:
                    triplet_spans = model.extract(batch_tokens,
                                                  sub_threshold=args.sub_threshold-0.2,
                                                  obj_threshold=args.obj_threshold-0.1)
                if len(triplet_spans) == 0:
                    triplet_spans = model.extract(batch_tokens,
                                                  sub_threshold=args.sub_threshold-0.2,
                                                  obj_threshold=args.obj_threshold-0.2)
                if len(triplet_spans) == 0:
                    triplet_spans = model.extract(batch_tokens,
                                                  sub_threshold=args.sub_threshold-0.3,
                                                  obj_threshold=args.obj_threshold-0.2)
                if len(triplet_spans) == 0:
                    triplet_spans = model.extract(batch_tokens,
                                                  sub_threshold=args.sub_threshold-0.3,
                                                  obj_threshold=args.obj_threshold-0.3)
            ...
            for tri in triplet_spans:
                ...
                sub = ' '.join(tokens[tri[0][0]: tri[0][1]]).replace('##', '').\
                            ...
                            replace(' -', '-').replace('- ', '-'). \
                            replace(' –', '–').replace('– ', '–').\
                            replace(qmark_placeholder, '?x').replace(uri_placeholder, '?uri').lower()    # for the first 3 dataset ?uri
...
def train_and_evaluate(model, data_loader, train_data, dev_data, optimizer, args, ema=None, model_teacher=None):
    ...
    for epoch in range(1, args.epoch_num + 1):
        ...
        if epoch >= min(args.epoch_num // 10, 30) and epoch % 5 == 0:   # do eval after xx epochs, eval every x epoch
...
if __name__ == '__main__':
    ...
    if not args.debug:
        data_names = ['train_dev_re_pp', 'test_re_pp', 'test_re_pp'] # train_dev_2_part_verb_re_pp, test_2_re_pp, train_dev_re_pp
    ...
    if args.do_train_and_eval:
        train_data = data_loader.load_data(data_names[0], max_len=args.max_len,
                                           repeat_multi_sub=args.repeat_multi_sub,
                                           encoder_type=args.encoder_type)
        dev_data = data_loader.load_data(data_names[1], encoder_type=args.encoder_type)
        # Prepare model
        model = RnnNTR(args, num_class=len(args.idx2pre))
        glove = load_glove_model(args.word_embedding, dim=300)
        # glove = load_w2v_model(args.word_embedding, dim=300)
        model.encoder.create_emb_layer(data_loader, embedding_dict=glove, embedding_dim=300)
    ...
    if args.do_eval:
        ...
        # Define the model
        model = RnnNTR(args, num_class=len(args.idx2pre))
        glove = load_w2v_model(args.word_embedding, dim=300) # 42, 840
        model.encoder.create_emb_layer(data_loader, embedding_dict=glove, embedding_dim=300)
        model.to(args.device)
        # loading the best model
        utils.load_checkpoint(os.path.join(args.model_dir, 'best.pth.tar'), model)
        # inference test file
        test_data = data_loader.load_data(data_names[2], encoder_type=args.encoder_type)
